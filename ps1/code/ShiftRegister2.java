 ...
 */
public class ShiftRegister2 implements ILFShiftRegister {
    ///////////////////////////////////
    ...
    ///////////////////////////////////
    ShiftRegister2(int size, int tap) {
        // TODO:
        _seed=new int [size];
//        this.tap=tap;
        this.tap=size-tap-1;
...
//        return least_sig;
        int length=_seed.length;
        int tapBit=_seed[this.tap];
        int least_sig = _seed[0] ^ tapBit;    //XOR: results in 1 iff the two inputs are diff
        for (int i=0; i< _seed.length-1; i++) {
            _seed[i]=_seed[i+1];
        }
        _seed[_seed.length-1]=least_sig;
        return _seed[_seed.length-1];
