    ...
    public boolean checkBalance(TreeNode node) {
        // TODO: Implement this
         if (node != null) {
             ...
             boolean currentLevel = (leftWeight <= twoOverThree * node.weight)
                                     && (rightWeight <= twoOverThree * node.weight);
             return currentLevel && checkBalance(node.left) && checkBalance(node.right);
         } else {
             return true;
         }
    }
